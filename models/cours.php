<?php
    ...
    class Cours extends Database
    {
        ...
        public function takeCours($status, $id)
        {
            $bdd = $this -> dbConnect();
            $sql = $bdd -> prepare ('
                    SELECT id_'.$status.', nom_'.$status.' FROM '.$status.' 
                    WHERE id_'.$status.'= ?');
        ...
        public function updateCours($status, $id, $idCat, $idProf, $nom)
        {
            $bdd= $this -> dbConnect();
            $sql = $bdd -> prepare ('
                UPDATE '.$status.' SET nom_'.$status.'= ?, id_cat=?, id_professeur= ? 
                WHERE id_'.$status.'= ?');
        ...
        public function getCoursid($idProf)
        {
            ...
            $sql = $bdd->prepare('
                ...
                FROM cours 
                JOIN categorie 
                ON cours.id_cat=categorie.id_cat 
                JOIN professeur ON cours.id_professeur=professeur.id_professeur 
                WHERE professeur.id_professeur= ?');
